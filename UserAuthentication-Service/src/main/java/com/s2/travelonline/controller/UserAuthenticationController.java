package com.s2.travelonline.controller;
...
import com.s2.travelonline.service.UserAuthenticationService;
		...
		try {
			...
			if (returnuser != null) {
				...
				try {
					...
					this.authicationService.saveUser(returnuser);
	...
	isAuthenticated(@RequestHeader(name = "Authorization", required = false) String authHeader) {
		...
		try {
			...
			if(token!=null){
				...
				System.out.println("Bearer" + token.toString());
			return new ResponseEntity<Map<String,Boolean>>(responsemap, HttpStatus.OK);
			}else{
